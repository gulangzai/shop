package com.shop.business.buattachment.controller; 
...
/** 
 *  BuAttachmentController
 * 
 */
@Controller
@RequestMapping(value="/buattachment/buAttachmentController")
public class BuAttachmentController extends BaseController {
	
	String menuUrl = "buattachment/buAttachmentController/list.do";  
	
	@Resource(name="buAttachmentService")
	private BuAttachmentService buAttachmentService;
	...
	public ResultAction save() throws Exception{
		logBefore(logger, "BuAttachmentController");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();     
		int ID = buAttachmentService.save(pd);
		...
		if(!"".equals(PICTURES_IDs)){
			for(int i=0;i<PICTURES_IDs.length;i++){
				...
				picture.put("TABLE_NAME", "BU_ATTACHMENT");			 
	...
	public ModelAndView list(Page page){
		...
		ModelAndView mv = this.getModelAndView();
		try{
		 	mv.setViewName("business/buattachment/bu_attachment_index"); 
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	...
	public Page dataList(){
		...
		try{ 
			page.setPd(pd);  
			varList = buAttachmentService.list(page);	 
	...
	public ModelAndView goAdd(){ 
		...
		pd = this.getPageData();
		try {
			mv.setViewName("business/buattachment/bu_attachment_add");
	...
	public PageData goEdit(){
		...
		pd = this.getPageData();
		try {
			pd = buAttachmentService.findById(pd);	 
		} catch (Exception e) {
			logger.error(e.toString(), e);
		}						
		return pd;
	...
	public ModelAndView goEditView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/buattachment/bu_attachment_edit"); 
	...
	public ModelAndView goDetailView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/buattachment/bu_attachment_detail"); 
			mv.addObject("msg", "edit"); 
			mv.addObject("pd",pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
	...
	public Object deleteAll() { 
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				buAttachmentService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
