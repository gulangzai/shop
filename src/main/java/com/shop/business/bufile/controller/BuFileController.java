package com.shop.business.bufile.controller; 
 ...
 */
@Controller
@RequestMapping(value="/bufile/buFileController")
public class BuFileController extends BaseController {
	
	String menuUrl = "bufile/buFileController/list.do";  
	
	@Resource(name="buFileService")
	private BuFileService buFileService;
	...
	public ResultAction save() throws Exception{
		logBefore(logger, "BuFileController");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();     
		int ID = buFileService.save(pd);
		...
		if(!"".equals(PICTURES_IDs)){
			for(int i=0;i<PICTURES_IDs.length;i++){
				...
				picture.put("TABLE_NAME", "BU_FILE");			 
	...
	public ModelAndView list(Page page){
		...
		ModelAndView mv = this.getModelAndView();
		try{
		 	mv.setViewName("business/bufile/bu_file_index"); 
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	...
	public Page dataList(){
		...
		try{ 
			page.setPd(pd);  
			varList = buFileService.list(page);	 
	...
	public ModelAndView goAdd(){ 
		...
		pd = this.getPageData();
		try {
			mv.setViewName("business/bufile/bu_file_add");
	...
	public ModelAndView goEditView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/bufile/bu_file_edit"); 
	...
	public ModelAndView goDetailView(){
		...
		pd = this.getPageData();
		try { 
		    mv.setViewName("business/bufile/bu_file_detail"); 
			mv.addObject("msg", "edit"); 
			mv.addObject("pd",pd);
		} catch (Exception e) {
			logger.error(e.toString(), e);
	...
	public Object deleteAll() { 
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				buFileService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
