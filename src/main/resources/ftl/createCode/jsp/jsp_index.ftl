...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid"> 
	<div class="col-sm-12" id="inside">
			...
			<form class="form-inline" width="100%" action="<#noparse>${ctx}</#noparse>/${controllerName}/list.do" method="post" name="INDEX_Form" id="INDEX_Form" style="border:solid 1px #ddd;line-height:50px;vertical-align:middle;">
		 	...
		 	<table id="jqGrid" class="table table-striped table-bordered table-hover imagetable"  action="<#noparse>${ctx}</#noparse>/${controllerName}/dataList.do">
	...
	<!-- PAGE CONTENT ENDS HERE -->
       ...
       <script type="text/javascript">
		 ...
		 function doReportInfo(cellvalue, opts, rowObject){
    	    var ${PK_Name} = rowObject.${PK_Name}; 
    		
    		var showHtml =  "<button type='button' class='btn btn-default btn-sm' onclick='see("+${PK_Name}+")'>";
    		showHtml +="<span class='glyphicon  glyphicon glyphicon-file'></span>";
    		showHtml +="</button>";
    		
    		showHtml +=  "<button type='button' class='btn btn-default btn-sm' onclick='edit("+${PK_Name}+")'>";
    		showHtml +="<span class='glyphicon glyphicon-edit'></span>";
    		showHtml +="</button>";
		
    		showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='del("+${PK_Name}+")'>";
		...
		function add(id){
		  ...
		  $.get("<#noparse>${ctx}</#noparse>/${controllerName}/goAddView.do?${table.key}="+id,function(data) {
		    	$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});
		...
		function edit(Id){
		    ...
		    $.get("<#noparse>${ctx}</#noparse>/${controllerName}/goEditView.do?${table.key}="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
	   ...
	   function see(Id){
		    ...
		    $.get("<#noparse>${ctx}</#noparse>/${controllerName}/goDetailView.do?${table.key}="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					...
					var url = "<#noparse>${ctx}</#noparse>/${controllerName}/delete.do?${table.key}="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<#noparse>${ctx}</#noparse>/${controllerName}/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								...
								success: function(data){
									 $.each(data.list, function(i, list){
											nextPage(<#noparse>${page.currentPage}</#noparse>);
									 });
								}
							});
						}
		...
		//导出excel
		function toExcel(){
			window.location.href='<#noparse>${ctx}</#noparse>/${controllerName}/excel.do';
		}
		</script>
