 ...
 * 表格生成类
 */
var JqTableManage  = new Object();	

/**
 * tableid： jqgrid的table
 * queryformid：查询条件的formid
 * */
JqTableManage.initJqgrid=function(tableid,queryformid,gridwidth){
//alert("tableid==="+tableid+"======queryformid==="+queryformid);
		...
		$(jqgridtableid).each(function(){
			...
			var json=JqTableManage.jqgridconfig(this.id);	
			   ...
			   datatype : function(postdata) {
			       ...
			       jQuery.ajax({   
			    	...
			    	//   alert(resp.msg);
			            	...
			            	JqTableManage.updatePagerIcons(null);
			            }
			        });  
			        //jqgrid宽度自适应
	           ...
	           jsonReader : { 
				...
				loadComplete : function(data) {
					...
					setTimeout(function(){
						...
						JqTableManage.updatePagerIcons(table);
					}, 0);
 ...
 * @param {} gridwidth
 */
JqTableManage.initJqgrid2=function(tableid,queryformid,gridwidth){
//alert("tableid==="+tableid+"======queryformid==="+queryformid);
	var jqgridtableid;
	if(tableid && typeof(tableid) == "object"){   
		jqgridtableid = tableid;
 ...
 * @param {} gridwidth
 */
JqTableManage.initHaveDblClickJqgrid=function(tableid,queryformid,gridwidth){
//alert("tableid==="+tableid+"======queryformid==="+queryformid);
	var jqgridtableid;
	if(tableid && typeof(tableid) == "object"){   
		jqgridtableid = tableid;
...
JqTableManage.updatePagerIcons=function(table) {
...
JqTableManage.jqgridconfig=function(id){
	if($("#"+id).length==1){
...
//影藏列
JqTableManage.hideRow = function (tableId){
...
//影藏行
JqTableManage.hiddenLine = function(Col,storeId,slected){
	...
	for(var i=1;i<rowsLength;i++){//按表的行数进行循环，本例第一行是标题，所以i=1，从第二行开始筛选（从0数起）
		...
		if(JqTableManage.contains(arr,i)){
...
//判断某个元素是否在指定的数组中存在
JqTableManage.contains = function (array ,element){
...
//去重
JqTableManage.sortUnique =  function(arr) {
	arr.sort(function(a, b){
		a = JqTableManage.adjustCase(a);
		b = JqTableManage.adjustCase(b);	
		if (a == b) return 0;
		return a > b ? 1 : -1;
	});
	...
	for (var i = 1; i < arr.length; i++) { // start loop at 1 as element 0 can never be a duplicate
		if (JqTableManage.adjustCase(arr[i-1]) !== JqTableManage.adjustCase(arr[i])) {
			ret.push(arr[i]);
		}
	}
	return ret;
...
JqTableManage.adjustCase = function(val) {
 ...
 * @param {} Url 查询url
 */
JqTableManage.Autocomplete = function(inputId, Url) {
	$('#' + inputId).typeahead({
