...
var contextPath;
//初始化 swf
function initSwfupload(url,sizeLimit,types,typsdesc,uploadLimit,postdata,contextPath){
		contextPath=contextPath;
		//初始化界面
		initswfct(uploadLimit,sizeLimit);
		//初始化 swfupload
		var settings={
			// Flash Settings flash9_url :"http://192.168.1.115:8080/bjsearch/js/plugins/multiimage/js/swfupload_fp9.swf",
			flash_url :contextPath+"/assets/sys_resources/plugins/swfupload/js/swfupload.swf",
			...
			// Button Settings   swfupload images
			button_image_url : contextPath+"/assets/sys_resources/plugins/swfupload/images/button_notext.png",
...
//初始化 界面
function initswfct(uploadLimit,sizeLimit){
	...
	var uploadbody='<div id="uploadBody" class="ke-dialog-upload">'
		...
		+'  <div style="float: left">批量文件上传</div><div id="iconClose" class="ke-dialog-icon-close" title="关闭"></div><div style="clear: both"></div>'
		+'	</div>'
		+'	<!--content-->'
		...
		+'		  <div class="ke-inline-block ke-swfupload-desc">允许用户同时上传文件数&nbsp;<font color="red">'+uploadLimit+'</font>，单个文件大小不超过<font color="red">'+sizeLimit+'</font></div>'
		...
		+'	</div>'
		+' <div style="display: block; width: 100%; height: 1000px; position: absolute; left: 0px; top: 0px; z-index: 811212;" class="ke-dialog-mask"></div>';
...
function iconClose(event){
	...
	queryFileDataSwf();//关闭后调用此方法，查询符合条件的附件
}
...
function addReadyFileInfo(fileid,fileName,message){
	var srcpath=contextPath+"/assets/sys_resources/plugins/swfupload/images/image.png";
//	alert("---------------"+srcpath);
	var photoDiv='<div class="ke-inline-block ke-item" id="'+fileid+'">'
/**	    +'<div class="ke-inline-block ke-photo" >'
	   // +'	<img src="'+srcpath+'" class="ke-img" data-status="-1" width="80" height="80" />'
