...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid"> 
	...
	<div class="col-sm-12" id="inside">
			...
			<!-- 检索  -->
			<form class="form-inline" width="100%" action="${ctx}/activitiCtrl/act_re_procdef_list_dataList.do" method="post" name="INDEX_Form" id="INDEX_Form" style="border:solid 1px #ddd;line-height:50px;vertical-align:middle;">
 	...
 	<table id="jqGrid" class="table table-striped table-bordered table-hover imagetable" action="${ctx}/activitiCtrl/act_re_procdef_list_dataList.do">
		    <tr>
						<th class="center">
       ...
       function doReportInfo(cellvalue, opts, rowObject){
    	...
    	/*	var showHtml ="<div class='hidden-sm hidden-xs action-buttons' style='padding-top:15px;'>";	
    		...
    		*/
    		...
    		//showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='start("+ID_+")'>";
    		showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='start()'>";
		...
		function start(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) { 
					...
					$.ajax({
						type: "POST",
						url: "${ctx}/activitiCtrl/start.do?ID_="+Id+"&tm="+new Date().getTime(),
				         dataType:'json',
						//beforeSend: validateData,
						cache: false,
						success: function(response){ 
							 if(!response.iserror){
									xAlert("信息提示","启动流程成功",1); 
									$("#INDEX_Form").submit();
							 }  
						}
					});
