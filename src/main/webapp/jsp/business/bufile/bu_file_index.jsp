...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid"> 
	<div class="col-sm-12" id="inside">
			...
			<form class="form-inline" width="100%" action="${ctx}/bufile/buFileController/list.do" method="post" name="INDEX_Form" id="INDEX_Form" style="border:solid 1px #ddd;line-height:50px;vertical-align:middle;">
		 	...
		 	<table id="jqGrid" class="table table-striped table-bordered table-hover imagetable"  action="${ctx}/bufile/buFileController/dataList.do">
		        
		        <tr>
		          
							    <th name="TARGET_TABLE"   align="center" formatter="">附件所属对象表名</th> 
							    <th name="TARGET_COL"   align="center" formatter="">附件所属对象列名</th> 
							    <th name="TARGET_UID"   align="center" formatter="">附件所属对象ID</th> 
							    <th name="FILE_TYPE"   align="center" formatter="">文件类型</th> 
							    <th name="FILE_TYPE_NAME"   align="center" formatter="">附件类别名称</th> 
							    <th name="DESCRIPTION"   align="center" formatter="">附件的描述</th> 
							    <th name="ATTACHMENT_UID"   align="center" formatter="">附件ID</th> 
							    <th name="ENABLED"   align="center" formatter="">是否有效</th> 
	...
	<!-- PAGE CONTENT ENDS HERE -->
       ...
       <script type="text/javascript">
		...
		function add(id){
		  ...
		  $.get("${ctx}/bufile/buFileController/goAddView.do?FILE_UID="+id,function(data) {
		    	$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});
		...
		function edit(Id){
		    ...
		    $.get("${ctx}/bufile/buFileController/goEditView.do?FILE_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
	   ...
	   function see(Id){
		    ...
		    $.get("${ctx}/bufile/buFileController/goDetailView.do?FILE_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					...
					var url = "${ctx}/bufile/buFileController/delete.do?FILE_UID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '${ctx}/bufile/buFileController/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='${ctx}/bufile/buFileController/excel.do';
		}
		</script>
