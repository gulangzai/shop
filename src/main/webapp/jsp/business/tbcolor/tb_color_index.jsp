...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid"> 
	<div class="col-sm-12" id="inside">
			...
			<form class="form-inline" width="100%" action="${ctx}/tbcolor/tbColorController/list.do" method="post" name="INDEX_Form" id="INDEX_Form" style="border:solid 1px #ddd;line-height:50px;vertical-align:middle;">
		 	...
		 	<table id="jqGrid" class="table table-striped table-bordered table-hover imagetable"  action="${ctx}/tbcolor/tbColorController/dataList.do">
		        
		        <tr>
		          
							    <th name="F_PRODUCT_ID"   align="center" formatter="">商品表UID</th> 
							    <th name="F_COLOR_LABEL"   align="center" formatter="">颜色名</th> 
							    <th name="F_XUHAO"   align="center" formatter="">序号</th> 
							    <th name="F_STATUS"   align="center" formatter="">状态</th> 
	...
	<!-- PAGE CONTENT ENDS HERE -->
       ...
       <script type="text/javascript">
		 ...
		 function doReportInfo(cellvalue, opts, rowObject){
    	    var F_COLOR_UID = rowObject.F_COLOR_UID; 
    		
    		var showHtml =  "<button type='button' class='btn btn-default btn-sm' onclick='see("+F_COLOR_UID+")'>";
    		showHtml +="<span class='glyphicon  glyphicon glyphicon-file'></span>";
    		showHtml +="</button>";
    		
    		showHtml +=  "<button type='button' class='btn btn-default btn-sm' onclick='edit("+F_COLOR_UID+")'>";
    		showHtml +="<span class='glyphicon glyphicon-edit'></span>";
    		showHtml +="</button>";
		
    		showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='del("+F_COLOR_UID+")'>";
		...
		function add(id){
		  ...
		  $.get("${ctx}/tbcolor/tbColorController/goAddView.do?F_COLOR_UID="+id,function(data) {
		    	$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});
		...
		function edit(Id){
		    ...
		    $.get("${ctx}/tbcolor/tbColorController/goEditView.do?F_COLOR_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
	   ...
	   function see(Id){
		    ...
		    $.get("${ctx}/tbcolor/tbColorController/goDetailView.do?F_COLOR_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					...
					var url = "${ctx}/tbcolor/tbColorController/delete.do?F_COLOR_UID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '${ctx}/tbcolor/tbColorController/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='${ctx}/tbcolor/tbColorController/excel.do';
		}
		</script>
