...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid"> 
	<div class="col-sm-12" id="inside">
			...
			<form class="form-inline" width="100%" action="${ctx}/tbmessage/tbMessageController/list.do" method="post" name="INDEX_Form" id="INDEX_Form" style="border:solid 1px #ddd;line-height:50px;vertical-align:middle;">
		 	...
		 	<table id="jqGrid" class="table table-striped table-bordered table-hover imagetable"  action="${ctx}/tbmessage/tbMessageController/dataList.do">
		        
		        <tr>
		          
							    <th name="F_USER_ID"   align="center" formatter="">用户账号</th> 
							    <th name="F_TITLE"   align="center" formatter="">主题</th> 
							    <th name="F_CONTENT"   align="center" formatter="">内容</th> 
	...
	<!-- PAGE CONTENT ENDS HERE -->
       ...
       <script type="text/javascript">
		 ...
		 function doReportInfo(cellvalue, opts, rowObject){
    	    var F_MESSAGE_UID = rowObject.F_MESSAGE_UID; 
    		
    		var showHtml =  "<button type='button' class='btn btn-default btn-sm' onclick='see("+F_MESSAGE_UID+")'>";
    		showHtml +="<span class='glyphicon  glyphicon glyphicon-file'></span>";
    		showHtml +="</button>";
    		
    		showHtml +=  "<button type='button' class='btn btn-default btn-sm' onclick='edit("+F_MESSAGE_UID+")'>";
    		showHtml +="<span class='glyphicon glyphicon-edit'></span>";
    		showHtml +="</button>";
		
    		showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='del("+F_MESSAGE_UID+")'>";
		...
		function add(id){
		  ...
		  $.get("${ctx}/tbmessage/tbMessageController/goAddView.do?F_MESSAGE_UID="+id,function(data) {
		    	$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});
		...
		function edit(Id){
		    ...
		    $.get("${ctx}/tbmessage/tbMessageController/goEditView.do?F_MESSAGE_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
	   ...
	   function see(Id){
		    ...
		    $.get("${ctx}/tbmessage/tbMessageController/goDetailView.do?F_MESSAGE_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					...
					var url = "${ctx}/tbmessage/tbMessageController/delete.do?F_MESSAGE_UID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '${ctx}/tbmessage/tbMessageController/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='${ctx}/tbmessage/tbMessageController/excel.do';
		}
		</script>
