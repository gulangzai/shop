...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid"> 
	<div class="col-sm-12" id="inside">
			...
			<form class="form-inline" width="100%" action="${ctx}/tbsize/tbSizeController/list.do" method="post" name="INDEX_Form" id="INDEX_Form" style="border:solid 1px #ddd;line-height:50px;vertical-align:middle;">
		 	...
		 	<table id="jqGrid" class="table table-striped table-bordered table-hover imagetable"  action="${ctx}/tbsize/tbSizeController/dataList.do">
		        
		        <tr>
		          
							    <th name="F_PRODUCT_ID"   align="center" formatter="">商品表UID</th> 
							    <th name="F_SIZE_LABEL"   align="center" formatter="">尺寸名</th> 
	...
	<!-- PAGE CONTENT ENDS HERE -->
       ...
       <script type="text/javascript">
		 ...
		 function doReportInfo(cellvalue, opts, rowObject){
    	    var F_SIZE_UID = rowObject.F_SIZE_UID; 
    		
    		var showHtml =  "<button type='button' class='btn btn-default btn-sm' onclick='see("+F_SIZE_UID+")'>";
    		showHtml +="<span class='glyphicon  glyphicon glyphicon-file'></span>";
    		showHtml +="</button>";
    		
    		showHtml +=  "<button type='button' class='btn btn-default btn-sm' onclick='edit("+F_SIZE_UID+")'>";
    		showHtml +="<span class='glyphicon glyphicon-edit'></span>";
    		showHtml +="</button>";
		
    		showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='del("+F_SIZE_UID+")'>";
		...
		function add(id){
		  ...
		  $.get("${ctx}/tbsize/tbSizeController/goAddView.do?F_SIZE_UID="+id,function(data) {
		    	$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});
		...
		function edit(Id){
		    ...
		    $.get("${ctx}/tbsize/tbSizeController/goEditView.do?F_SIZE_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
	   ...
	   function see(Id){
		    ...
		    $.get("${ctx}/tbsize/tbSizeController/goDetailView.do?F_SIZE_UID="+Id,function(data) {
				$("#edit-form").empty();
				$("#edit-form").html("");
				$("#edit-form").html(data);
			});   
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					...
					var url = "${ctx}/tbsize/tbSizeController/delete.do?F_SIZE_UID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '${ctx}/tbsize/tbSizeController/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='${ctx}/tbsize/tbSizeController/excel.do';
		}
		</script>
