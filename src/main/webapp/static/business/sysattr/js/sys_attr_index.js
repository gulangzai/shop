...
$(top.hangge());
 ...
 $(window).on(
   ...
   function doReportInfo(cellvalue, opts, rowObject){
	   var F_ATTR_UID = rowObject.F_ATTR_UID; 
		var showHtml =  "<button type='button' class='btn btn-default btn-sm' onclick='edit("+F_ATTR_UID+")'>";
	showHtml +="<span class='glyphicon glyphicon-edit'></span>";
	showHtml +="</button>";
	showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='del("+F_ATTR_UID+")'>";
...
function add(){ 
   $('#edit').attr("data-target","edit-form");
	...
	$.get(ctx+"/sysattr/sysAttrController/goAddView.do",function(data) {
		$("#edit-form").empty();
		$("#edit-form").html("");
		$("#edit-form").html(data);
	}); 
...
function del(Id){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) {  
			$.ajax({
				type: "POST",
				url: ctx+"/sysattr/sysAttrController/delete.do?F_ATTR_UID="+Id+"&tm="+new Date().getTime(),
		         dataType:'json',
		...
		bootbox.confirm(msg, function(result) {
			...
			if(str==''){ 
				...
				if(msg == '确定要删除选中的数据吗?'){ 
					$.ajax({
						type: "POST",
						url: ctx+'/sysattr/sysAttrController/deleteAll.do?tm='+new Date().getTime(),
				    	data: {DATA_IDS:str},
						dataType:'json', 
						cache: false,
...
function edit(Id){
   ...
   $.get(ctx+"/sysattr/sysAttrController/goEditView.do?F_ATTR_UID="+Id,function(data) {
			$("#edit-form").empty();
			$("#edit-form").html("");
			$("#edit-form").html(data);
    });  
...
//导出excel
function toExcel(){
	window.location.href= ctx+'/sysattr/sysAttrController/excel.do';
}
