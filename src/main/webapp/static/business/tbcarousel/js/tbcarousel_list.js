...
function doReportInfo(cellvalue, opts, rowObject){
	...
	showHtml +=  "<button type='button' class='btn btn-default btn-sm' onclick='edit("+F_Id+")'>";
	showHtml +="<span class='glyphicon glyphicon-edit'></span>";
	showHtml +="</button>";

	showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='del("+F_Id+")'>";
...
function FNonticeTitleFormatter(cellvalue, opts, rowObject){
    ...
    var F_Id = rowObject.F_Id;
	var showHtml = "";
	showHtml +="<span class='glyphicon'><a href='javascript:void(0)' onclick='see("+F_Id+")'>"+F_NOTICE_TITLE+"</a></span>";  
	return 	showHtml;
}
...
function add(id){
  ...
  $.get(ctx+"/tbcarousel/tbCarouselController/goAddView.do?F_Id="+id,function(data) {
    	$("#edit-form").empty();
		$("#edit-form").html("");
		$("#edit-form").html(data);
	});
...
function edit(Id){
    ...
    $.get(ctx+"/tbcarousel/tbCarouselController/goEditView.do?F_Id="+Id,function(data) {
		$("#edit-form").empty();
		$("#edit-form").html("");
		$("#edit-form").html(data);
		});   
   ...
   function see(Id){
    ...
    $.get(ctx+"/tbcarousel/tbCarouselController/goDetailView.do?F_Id="+Id,function(data) {
		$("#edit-form").empty();
		$("#edit-form").html("");
		$("#edit-form").html(data);
	});   
...
function del(Id){
	bootbox.confirm("确定要删除吗?", function(result) {
		if(result) { 
			var url = ctx+"/tbcarousel/tbCarouselController/delete.do?F_Id="+Id+"&tm="+new Date().getTime();
		...
		if(result) { 
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){ 
					$.ajax({
						type: "POST",
						url: ctx+'/tbcarousel/tbCarouselController/deleteAll.do?tm='+new Date().getTime(),
				    	data: {DATA_IDS:str},
						dataType:'json', 
						cache: false,
...
//导出excel
function toExcel(){
	window.location.href=ctx+'/tbcarousel/tbCarouselController/excel.do';
}
