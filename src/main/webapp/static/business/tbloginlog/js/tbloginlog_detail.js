$(function(){ 
 ...
 function init(){ 
		var data1={F_LOGIN_LOG_UID: $("#F_LOGIN_LOG_UID").val()};
		$.ajax({
			url : ctx+"/tbloginlog/tbLoginLogController/goEdit.do",
			data : data1,
			cache : false,
			async :	false,
			dataType : "json",  
	...
	function setValue(obj){  
		 $("#F_LOGIN_LOG_UID").val(obj.F_LOGIN_LOG_UID); 
	     $("#userId").val(obj.userId); 
	     $("#F_UserName").val(obj.F_UserName); 
	     $("#loginTime").val(obj.loginTime); 
	     $("#loginOut").val(obj.loginOut);  
    }
