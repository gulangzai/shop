...
function doReportInfo(cellvalue, opts, rowObject){
	var F_LOGIN_LOG_UID = rowObject.F_LOGIN_LOG_UID;  
	var showHtml = "";
	
	showHtml +=showHtml+"<button type='button' class='btn btn-default btn-sm' onclick='see("+F_LOGIN_LOG_UID+")'>";
...
function FUserNameFormatter(cellvalue, opts, rowObject){
    ...
    var F_LOGIN_LOG_UID = rowObject.F_LOGIN_LOG_UID;
	var showHtml = "";
	showHtml +="<span class='glyphicon'><a href='javascript:void(0)' onclick='see("+F_LOGIN_LOG_UID+")'>"+F_UserName+"</a></span>";  
	return 	showHtml;
}
...
function edit(Id){
   ...
   function see(Id){
    ...
    $.get(ctx+"/tbloginlog/tbLoginLogController/goDetailView.do?F_LOGIN_LOG_UID="+Id,function(data) {
		$("#edit-form").empty();
		$("#edit-form").html("");
		$("#edit-form").html(data);
	});   
		...
		if(result) { 
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){ 
					$.ajax({
						type: "POST",
						url: ctx+'/tbloginlog/tbLoginLogController/deleteAll.do?tm='+new Date().getTime(),
				    	data: {DATA_IDS:str},
						dataType:'json', 
						cache: false,
...
//导出excel
function toExcel(){
	window.location.href=ctx+'/tbloginlog/tbLoginLogController/excel.do';
}
