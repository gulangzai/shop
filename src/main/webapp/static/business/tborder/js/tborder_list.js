...
function doReportInfo(cellvalue, opts, rowObject){
	    var F_ORDER_UID = rowObject.F_ORDER_UID; 
	...
	showHtml +=  "<button type='button' class='btn btn-default btn-sm' onclick='edit("+F_ORDER_UID+")'>";
	showHtml +="<span class='glyphicon glyphicon-edit'></span>";
	showHtml +="</button>";

	showHtml +="<button type='button' class='btn btn-default btn-sm' onclick='del("+F_ORDER_UID+")'>";
...
function add(id){
  ...
  $.get(ctx+"/tborder/tbOrderController/goAddView.do?F_ORDER_UID="+id,function(data) {
    	$("#edit-form").empty();
		$("#edit-form").html("");
		$("#edit-form").html(data);
	});
...
function edit(Id){
    ...
    $.get(ctx+"/tborder/tbOrderController/goEditView.do?F_ORDER_UID="+Id,function(data) {
		$("#edit-form").empty();
		$("#edit-form").html("");
		$("#edit-form").html(data);
		});   
   ...
   function see(Id){
    ...
    $.get(ctx+"/tborder/tbOrderController/goDetailView.do?F_ORDER_UID="+Id,function(data) {
		$("#edit-form").empty();
		$("#edit-form").html("");
		$("#edit-form").html(data);
	});   
		...
		if(result) { 
			...
			if(str==''){
				...
				if(msg == '确定要删除选中的数据吗?'){ 
					$.ajax({
						type: "POST",
						url: ctx+'/tborder/tbOrderController/deleteAll.do?tm='+new Date().getTime(),
				    	data: {DATA_IDS:str},
						dataType:'json', 
						cache: false,
...
//导出excel
function toExcel(){
	window.location.href=ctx+'/tborder/tbOrderController/excel.do';
}
